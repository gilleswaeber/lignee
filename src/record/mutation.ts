	...
	updateItem({
		...
		update: (parent) => {
			if (parent == null) return { attr: { [selector.tag]: payload } };
				...
				return { value: parent, attr: { [selector.tag]: payload } };
			else
				return { ...parent, attr: { ...parent.attr, [selector.tag]: payload } };
		},
	});
}
		...
		update: (parent) => {
			...
			return {
				...
				attr: Object.fromEntries(
					Object.entries(parent.attr).filter(([k, _]) => k != selector.tag),
				),
			};
		},
	});
...
export function pushItems(
	...
	updateTag({
		...
		create: true,
		update: (it) =>
			matchTagPayload<Immutable<TagPayload>>(it, {
				missing: () => (payloads.length == 1 ? payloads[0] : payloads),
				array: (arr) =>
					arr.length == 0 && payloads.length == 1
						? payloads[0]
						: [...arr, ...payloads],

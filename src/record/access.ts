...
import {AssignOnNonExistentIndex, MissingItemError, UnexpectedError} from "./errors";
import type {GedcomRecord} from "../reader/models";

export function resolveItem(selector: ItemSelector): Immutable<ItemPayload> | null {
	let p: Immutable<ItemPayload> = selector.ctx.getRecord();
	for (const item of selector.path) {
		const [tag, index] = item;
		if (typeof p == "string") return null;
		if (!Object.hasOwn(p.attr, tag)) return null;
		const a: Immutable<TagPayload> | undefined = p.attr[tag];
		...
		if (Array.isArray(a)) {
			...
			p = a as Immutable<ItemPayload>;
		}
	}
	return p;
}

function deepItemUpdate(u: UpdateItem, it: Immutable<ItemPayload> | null, depth: number): Immutable<ItemPayload> {
	...
	return matchItemPayload<Immutable<ItemPayload>>(it, {
		...
		object: obj => matchTagPayload<Immutable<ItemPayload>>(obj.attr[tag], {
			single: a => {
				if (index > 0 && u.skipMissing) return obj;
				if (index == 1 && u.create) return {
					...obj,
					attr: {...obj.attr, [tag]: [a, deepItemUpdate(u, null, depth + 1)]}
				};
			...
			array: a => {
				...
				if (index == a.length && u.create) return {
					...obj,
					attr: {...obj.attr, [tag]: [...a, deepItemUpdate(u, null, depth + 1)]}
				};
				...
				return {
					...
					attr: {
						...obj.attr,
						[tag]: a.map((v, k) => k != index ? v : deepItemUpdate(u, a[index], depth + 1)),
					},
				};
			},
			missing: () => {
				...
				return {...obj, attr: {...obj.attr, [tag]: deepItemUpdate(u, null, depth + 1)}};
			}
		})
	})
}
...
export function matchItemPayload<R>(p: Immutable<ItemPayload> | null | undefined, h: {
	missing: () => R,
	string: (p: string) => R,
...
export function matchTagPayload<R>(p: Immutable<TagPayload> | null | undefined, h: {
	missing: () => R,
...
export function updateItem(update: UpdateItem): void {
	update.selector.ctx.setRecord(r => deepItemUpdate(update, r, 0) as Immutable<GedcomRecord>);
}

export function updateTag({selector, update, create, skipMissing}: UpdateTag): void {
	updateItem({
		selector,
		update: it => matchItemPayload<Immutable<ItemPayload>>(it, {
			...
			object: (it) => {
				if (Object.hasOwn(it.attr, selector.tag) && it.attr[selector.tag]) {
					return {...it, attr: {...it.attr, [selector.tag]: update(it.attr[selector.tag]!)}};
				} else {
					if (skipMissing) return it;
					if (!create) throw new MissingItemError(selector);

					return {...it, attr: {...it.attr, [selector.tag]: update(null)}};
				}
			}
		}),
		create,

...
import { RootTagsWithoutRef } from "../reader/records.js";
import type { GedcomRecord } from "../reader/models.js";
import type { TagPayload } from "../models.js";
...
export function dumpGedcom(
	lineage: Lineage,
	p: WriterSettings = DefaultWriterSettings,
): string {
	let data = "";
	data += toGEDCOMEntry(lineage.head ?? DEFAULT_HEAD_RECORD, p);
	for (const tag of lineage.tags) {
		for (const xref of lineage.byTag[tag]) {
			const entry = lineage.byXref[xref];
			data += toGEDCOMEntry(entry, p);
		}
	}
	for (const extraEntry of lineage.extraEntries) {
		data += toGEDCOMEntry(extraEntry, p);
	}
	data += toGEDCOMEntry(lineage.trailer ?? DEFAULT_TRAILER_RECORD, p);
	return data;
}

/** Convert an entry to a string using the GEDCOM format. */
export function toGEDCOMEntry(entry: GedcomRecord, p: WriterSettings): string {
	let str = "";
	if (RootTagsWithoutRef.includes(entry.tag)) {
		str += `0 ${entry.tag}${p.ln}`;
	} else {
		if (typeof entry.value === "string" && entry.value.length) {
			writeValue(0, entry.xref ?? "", `${entry.tag} ${entry.value}`, p);
		} else str += `0 ${entry.xref ?? ""} ${entry.tag}${p.ln}`;
	}
	...
	if (p.wrapText) {
		...
		for (const line of lines) {
			...
			while (encoded.length > 253 - (firstChunk ? 0 : conc.length)) {
				...
				str += UTF8_DECODER.decode(encoded.subarray(0, end)) + p.ln;
				encoded = encoded.subarray(end);
			}
			if (!firstChunk) str += concStr;
			str += UTF8_DECODER.decode(encoded) + p.ln;
		}
		return str;
	} else {
		return `${level} ${tag} ` + lines.join(`${p.ln}${level + 1} CONT `) + p.ln;
	}
}
